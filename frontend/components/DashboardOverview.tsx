        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-gray-800 p-4 rounded-lg shadow border border-gray-700">
                    ...
                    <div className="text-2xl font-bold text-white">{totalAssets.toLocaleString()} <span className="text-sm font-normal text-gray-500">KRW</span></div>
                </div>
                <div className="bg-gray-800 p-4 rounded-lg shadow border border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                ...
                <div className="bg-gray-800 p-4 rounded-lg shadow border border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                        <div className="text-gray-400 text-sm">Stock Value</div>
                        <TrendingUp className="w-4 h-4 text-purple-400" />
                    </div>
                    <div className="text-2xl font-bold text-white">{stockValue.toLocaleString()} <span className="text-sm font-normal text-gray-500">KRW</span></div>
                </div>
                <div className="bg-gray-800 p-4 rounded-lg shadow border border-gray-700">
                    <div className="flex items-center justify-between mb-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    {/* Asset Allocation Chart (Moved to Bottom, Compact) */}
                    <div className="bg-gray-800 p-4 rounded-lg shadow border border-gray-700 flex flex-col justify-between">
                        ...
                        <div className="h-40 relative w-full">
                            <AssetAllocationChart cash={userProfile.balance} stockValue={stockValue} usedCredit={usedCredit} />
                        </div>
                        <div className="flex justify-center flex-wrap gap-x-4 gap-y-2 mt-3 text-xs">
                            <div className="flex items-center space-x-1">
                                <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                                <span className="text-gray-400">Cash {totalAssets > 0 ? ((userProfile.balance / totalAssets) * 100).toFixed(0) : 0}%</span>
                            </div>
                            <div className="flex items-center space-x-1">
                                <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                                <span className="text-gray-400">Equity {totalAssets > 0 ? ((Math.max(0, stockValue - usedCredit) / totalAssets) * 100).toFixed(0) : 0}%</span>
                            </div>
                            {usedCredit > 0 && (
                                <div className="flex items-center space-x-1">
                                    <div className="w-2 h-2 rounded-full bg-red-500"></div>
                                    <span className="text-gray-400">Leverage {totalAssets > 0 ? ((Math.min(stockValue, usedCredit) / totalAssets) * 100).toFixed(0) : 0}%</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {/* Right Column (2 cols): AI Coach (Expanded) */}
